...
# 新增产品包装
def add_workpack_mode(packname, isdefault, Ptare, unit, Ctare, num):
    new_click(add)
    new_click(mater)
    new_click(mater_1)
...
# 编辑产品包装方式
def edit_workpack_mode(Ptare, Ctare, num):
    new_click(edit)
    sleep(2)
    new_click(default)
    new_click(pallet)
...
# 清除筛选结果
def clear_search_workpack_mode(text):
    new_click(search)
